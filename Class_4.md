...
Go to the last line of `linux` and add `rd.break`. [^1]
...
[^1]: When you add `rd.break` to your kernel boot parameters (in GRUB for example), it tells `dracut` to **break (pause) the boot process right before handing control over to the real root filesystem**.
	
	It drops you into an **emergency shell** inside the **initramfs environment**, which is a minimal root filesystem loaded into RAM for booting.
